...
const SYSTEM_PROMPT = `You are a senior distributional / drift analyst. Your toolkit:
- Kolmogorov-Smirnov two-sample test
- Population Stability Index (PSI): 0.1 = moderate shift, 0.25 = major shift
- Jensen-Shannon divergence; Wasserstein distance; Chi-square independence
...
FALLBACK: If orchestratorInstructions is null or empty, you MUST still perform a comprehensive distributional scan applying your full toolkit. Treat that case as "general" discoveryType.
...
Schema:
{
  "anomalies": [
    {
      "id": "D001",  // D001, D002, ... — required prefix
      "title": string,
      "evidence": string,
...
export async function distributionalAgent(csvString, inspectorProfile, orchestratorInstructions, userContext, apiKey, signal) {
...
Orchestrator instructions for the DISTRIBUTIONAL agent:
\`\`\`json
...
Apply BOTH prioritizedMethods and broaderScanMethods. Tag findings with discoveryType. Return only the JSON object.`;
  ...
  const text = extractAgentText(response, "Distributional");
